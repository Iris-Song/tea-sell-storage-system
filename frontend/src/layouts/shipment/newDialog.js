...
export default function NewDialog(props) {
  ...
  const handleSubmit = () => {
    ...
    let aPcs = [];
    aPcs.shipmentKey=[];
    aPcs.shipmentKey.product_no = pdcNoRef.current?.value;
    aPcs.shipmentKey.batch = batchRef.current?.value;
    aPcs.all_number = allNumRef.current?.value;
    aPcs.employee_no = eplIDRef.current?.value;
    aPcs.shipment_date = pcsDateRef.current?.value;
    aPcs.remarks = remarksRef.current?.value;
    props.addShipment(aPcs);
    setOpen(false);
  }
  ...
  return (
    <div>
      ...
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle>添加出货信息</DialogTitle>
        <DialogContent>
